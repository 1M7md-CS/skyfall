...
export default function Work() {
	...
	useEffect(() => {
		...
		const handleScreenSizeChange = (
			...
			setVisibleCount(
				work.map(() =>
					isDesktopScreen ? DESKTOP_INITIAL_CARDS : MOBILE_INITIAL_CARDS,
				),
			);
		};
	...
	const handleShowMore = (categoryIndex: number) => {
		setVisibleCount((prev) => {
			const updated = [...prev];
			const increment = isDesktop
				? DESKTOP_LOAD_MORE_COUNT
				: MOBILE_LOAD_MORE_COUNT;
			updated[categoryIndex] = Math.min(
				updated[categoryIndex] + increment,
				work[categoryIndex].cards.length,
			);
			return updated;
		});
	};
	...
	return (
		<div
			className="flex flex-col overflow-hidden pt-10 gap-10
                        lg:pt-20 xl:pt-[120px] xl:gap-[50px]"
		>
			{work.map((category, categoryIndex) => {
				...
				return (
					<section key={category.name} className="flex flex-col gap-12">
						...
						<div
							className="flex flex-col px-4 gap-[30px]
                                        lg:px-10 lg:gap-10
						...
						>
							...
							<div className="flex flex-col gap-[6px] lg:gap-[10px] xl:gap-3.5">
								...
								<p className="tracking-minus06 text-muted-foreground text-[14px] lg:text-base xl:text-[18px]">
									{category.summary}
								</p>
							</div>

							{/* Tagline */}
							<div className="bg-foreground max-w-fit p-2.5 rounded-md xl:rounded-lg xl:py-3 xl:px-3.5">
						...
						{hasMoreCards && (
							<div className="flex justify-center pb-[50px]">
								...
								>
									...
									<div
										className="absolute right-4 translate-x-10 opacity-0 transition-all
                                                    group-hover:translate-x-0 group-hover:opacity-100 flex"
									>
										<Image
											src="/assets/svgs/up-down-stars.svg"
											alt="icon"
											width={25}
											height={25}
